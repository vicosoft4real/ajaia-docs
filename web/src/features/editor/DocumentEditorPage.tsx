...
function LoadedEditor({ initialDocument, reload, onDeleted }: { initialDocument: DocumentDetail; reload: () => Promise<DocumentDetail | undefined>; onDeleted: () => void }) {
  ...
  return <section aria-labelledby="document-title" className="mx-auto max-w-5xl">
    ...
    <header className="mb-7 border-b border-border pb-6"><div className="flex flex-col gap-5 sm:flex-row sm:items-start sm:justify-between"><div className="min-w-0 flex-1"><p className="eyebrow">{document.isOwner ? "Owner · Your manuscript" : `Collaborator · Shared by ${document.owner.displayName}`}</p><input id="document-title" aria-label="Document title" readOnly={!document.canRename || saveState === "conflict"} value={title} onChange={(event) => setTitle(event.target.value)} onBlur={commitTitle} onKeyDown={(event) => { if (event.key === "Enter") event.currentTarget.blur(); }} className="mt-2 w-full border-0 bg-transparent p-0 font-[Literata] text-4xl font-semibold tracking-tight text-ink outline-none read-only:cursor-default focus:ring-0 sm:text-5xl" /></div>{document.isOwner && <div className="flex flex-wrap gap-2"><Button type="button" className="border border-border bg-white text-ink" onClick={() => setShareOpen(true)}><Share2 aria-hidden="true" size={16} />Share</Button><Button type="button" aria-label="Delete document" className="border border-border bg-white text-danger" onClick={() => setDeleteOpen(true)}><Trash2 aria-hidden="true" size={16} /></Button></div>}</div></header>
    ...
    {shareOpen && <ShareDocumentDialog documentId={document.id} open onOpenChange={setShareOpen} />}
  </section>;
}
