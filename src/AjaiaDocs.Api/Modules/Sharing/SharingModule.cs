...
public sealed class SharingModule : ICarterModule
{
    ...
    private static async Task<IResult> GetCandidatesAsync(Guid documentId,
        CurrentActor actor, GetShareCandidatesHandler handler, CancellationToken ct) =>
        (await handler.HandleAsync(actor.UserId, documentId, ct)).ToHttpResult();

    private static async Task<IResult> ListAsync(Guid id, CurrentActor actor,
        ListDocumentSharesHandler handler, CancellationToken ct) =>
        (await handler.HandleAsync(actor.UserId, id, ct)).ToHttpResult();

    private static async Task<IResult> GrantAsync(Guid id, GrantShareRequest request,
        CurrentActor actor, GrantDocumentShareHandler handler, CancellationToken ct) =>
        (await handler.HandleAsync(actor.UserId, id, request.UserId, ct))
        .ToHttpResult(StatusCodes.Status201Created);

    private static async Task<IResult> RevokeAsync(Guid id, Guid userId,
        CurrentActor actor, RevokeDocumentShareHandler handler, CancellationToken ct)
    {
